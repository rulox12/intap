                ...
                @foreach($activities as $activity)
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-center">
                            <span
                                class="bg-purple-200 text-purple-600 py-1 px-3 rounded-full text-xs">{{ $activity->created_at->diffForHumans() }}</span>
                        </td>
                        <td class="py-3 px-6 text-center">
                            <a href="{{route('activities.show', $activity->id)}}"
                               class="border-2 border-transparent bg-yellow-400 ml-3 py-1 px-4 font-bold text-white rounded transition-all hover:border-blue-500 hover:bg-transparent hover:text-blue-500">
                                {{ __('View') }}
                            </a>
                            <button onclick="openModalTime(true,{{ $activity->id }})"
                                    class="border-2 border-transparent bg-blue-400 ml-3 py-1 px-4 font-bold text-white rounded transition-all hover:border-blue-500 hover:bg-transparent hover:text-blue-500">
                                {{ __('Create new time') }}
                            </button>
                        </td>
...
<div id="modal_overlay-times"
    ...
    <div id="modal-times"
        ...
        <!-- header -->
        <div class="px-4 py-3 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-600">{{__('Create new time')}}</h2>
        </div>
        ...
        <form method="post" action="{{route('times.store')}}" enctype="multipart/form-data">
            ...
            <div class="p-2">
                <div class="mb-4 mt-6">
                    <input id="activity_id" name="activity_id" type="hidden" value="xm234jq"/>
                    <label
                        class="block text-gray-700 text-sm font-semibold mb-2"
                        htmlFor="minutes"
                    >
                    ...
                    <input
                        ...
                        required
                    />
                    <small>{{__('You can record maximum 480 minutes')}}</small>
...
<script>
    ...
    function openModalTime(value, time) {
        const modalCl = modal_times.classList
        const overlayCl = modal_overlay_times
        document.getElementById("activity_id").value = time
        if (value) {
            overlayCl.classList.remove('hidden')
            setTimeout(() => {
                modalCl.remove('opacity-0')
